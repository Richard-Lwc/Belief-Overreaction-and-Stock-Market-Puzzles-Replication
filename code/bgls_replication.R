...
# assume earnings are reported with a 3-month lag is not necessary.
...
sp500_DATA <- sp500_data %>% 
  ...
  mutate(EPS = epspi * ann_cfacshr / CFACSHR, 
         EPS_same_base = epspi * ann_cfacshr,
         MV = PRC * SHROUT,
         lag_MV = lag(MV)) %>% 
  filter(!is.na(MV))

sp500_DATA %>% fwrite("..\\overreactin&ML\\sp500.csv")
...
market_sp500 <- sp500_DATA %>% group_by(date, rankdate) %>% 
  summarise(LTG = weighted.mean(LTG, MV, na.rm = TRUE),
            ...
            EPS_same_base = sum(EPS_same_base * SHROUT, na.rm = TRUE), 
...
market_sp500 <- market_sp500 %>% 
  ...
  mutate(EPS = EPS / divisor, 
         EPS_same_base = EPS_same_base / divisor, 
         EPS_1 = EPS_1 / divisor,
         EPS_2 = EPS_2 / divisor,
         EPS_3 = EPS_3 / divisor,
         EPS_4 = EPS_4 / divisor,
  ...
  mutate(cae = EPS_same_base / CPILFESL) %>% ungroup() %>% 
  mutate(cae = lag(rollmean(cae, 12, fill = NA, align = 'right')) * CPILFESL,
         cae_m5 = lag(cae, 60)) %>% 
  filter(!is.na(LTG))


ggplot(market_sp500) + 
  # geom_line(mapping = aes(x = rankdate, y = log(EPS_1) - log(EPS)), colour = 'red') +
  geom_line(mapping = aes(x = rankdate, y = LTG))
...
market_sp500 <- market_sp500 %>% ungroup() %>% 
  mutate(YEAR = Year(date), QUARTER = Quarter(date),
         ...
         forecast_error = ifelse(EPS_same_base > 0, 
                                 (lead(EPS_same_base, 60) / EPS_same_base) ** (1 / 5) - 1 - LTG,
                                 ifelse(EPS_same_base < 0, 1 - (lead(EPS_same_base, 60) / EPS_same_base) ** (1 / 5) - LTG, NA)),
         e_cae = log(EPS_same_base) - log(cae_m5)) %>% 
...
market_all <- market_data %>% 
  ...
  mutate(Year = Year(STATPERS),
         LTG = LTG / 100, 
         # lag_bias = lag(bias, 12),
         # delta_bias = bias - lag_bias,
         lag_LTG = lag(LTG, 12), 
         delta_LTG = LTG - lag_LTG,
         EPS = epspi * ann_cfacshr, # 此处定义的EPS即为EPS_same_base
         MV = PRC * SHROUT, 
         lag_MV = lag(MV),
         log_RET = log(1 + RET),
         forecast_error = ifelse(EPS > 0, 
                                 (lead(EPS, 60) / EPS) ** (1 / 5) - 1 - LTG,
                                 ifelse(EPS < 0, 1 - (lead(EPS, 60) / EPS) ** (1 / 5) - LTG, NA)),
...
financial_ratio <- financial_ratio %>% mutate(rankdate = YearMonth(public_date)) %>% 
  select(permno, adate, rankdate, bm, pe_exi, pe_inc, divyield, oibd) %>% 
  rename(datadate = adate, PERMNO = permno, dp = divyield)
names(financial_ratio)
...
ff5_LTG <- na.omit(market_all %>% filter(!is.na(lag_MV)) %>% group_by(date, rankdate) %>% 
  summarise(LTG = weighted.mean(LTG, lag_MV, na.rm = TRUE))) %>% ungroup() %>% 
  mutate(lag_LTG = lag(LTG, 12), 
         delta_LTG = LTG - lag_LTG)

#######################         FF_5.csv is from Python          ###############
FF5 <- fread('FF_5.csv') %>% 
  filter(!is.na(size_rank), !is.infinite(forecast_error)) %>% 
  group_by(PERMNO) %>% 
...
f_SMB_BM <- BM %>% group_by(rankdate, size_rank) %>% 
  summarise(forecast_error = mean(forecast_error)) %>% 
